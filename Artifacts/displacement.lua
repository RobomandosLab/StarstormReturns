...
local validEnvs = List.new() -- this is where we will put all valid environments (no boar beach cuz its a secret and no risk of rain cuz its a final environemnt for example)
...
gm.post_script_hook(gm.constants.stage_roll_next, function(self, other, result, args)
	...
	if args[1].value == 1 then -- if its currently tier 1 >>
		potentialEnvs:clear() -- >> clear the list of potential envtronments
