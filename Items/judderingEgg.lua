local sprite_egg = Sprite.new("JudderingEgg", path.combine(PATH, "Sprites/Items/judderingEgg.png"), 1, 17, 18)
local sprite_wurm = Sprite.new("BabyWurm", path.combine(PATH, "Sprites/Items/Effects/babyWurm.png"), 5, 5, 13)

local judderingEgg = Item.new("judderingEgg")
judderingEgg:set_sprite(sprite_egg)
judderingEgg:set_tier(ItemTier.RARE)
judderingEgg.loot_tags = Item.LootTag.CATEGORY_DAMAGE + Item.LootTag.ITEM_BLACKLIST_ENGI_TURRETS

ItemLog.new_from_item(judderingEgg)
...
local function find_target(wurm)
	...
	if Instance.exists(candidate) and Math.distance(wurm.parent.x, wurm.parent.y, candidate.x, candidate.y) < WURM_ATTACK_RANGE then
...
Callback.add(judderingEgg.on_acquired, function(actor, stack)
...
Callback.add(judderingEgg.on_removed, function(actor, stack)
	local data = Instance.get_data(actor)
...
local sparkParticle = Particle.find("Spark")

local packet = Packet.new("SyncJudderingEgg")

local serializer = function(buffer, actor, index, target)
	buffer:write_instance(actor)
	buffer:write_uint(index)
	buffer:write_instance(target)
end
...
Callback.add(Callback.ON_STEP, Callback.Priority.AFTER, function()
	...
	for id, _ in pairs(wurm_owners) do
		...
		local stack = actor:item_count(judderingEgg)
		...
		for i, wurm in ipairs(wurm_pets) do
			...
			local dist = Math.distance(head.x, head.y, target_x, target_y)
			...
			if dist < WURM_FOLLOW_RADIUS + WURM_FOLLOW_MARGIN then
				...
				steer_direction = Math.sign(gm.angle_difference(head.direction, target_direction))
			...
			wurm.speed = Math.lerp(wurm.speed, target_speed, 0.01)
			...
			for i = 2, #wurm.segments do
				...
				seg.direction = Math.direction(seg.x, seg.y, seg_prev.x, seg_prev.y)
			...
			if wurm.cooldown < 0 then
				...
				if Net.host then
					...
					elseif not Instance.exists(wurm.target) then
						wurm.target = find_target(wurm)
						
						if not Instance.exists(wurm.target) then
							wurm.firing = false
							wurm.target = Instance.wrap()
							wurm.shots = 0
				...
				if Net.host and Net.online and sync then
					packet:send_to_all(actor, i - 1, wurm.target)
				end
			end
		end
	end
